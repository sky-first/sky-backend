...
async def backfill():
    ...
    async with async_session() as db:
        ...
        stmt = select(ChatMessage).where(ChatMessage.planet_id == None)
        ...
        for user in users:
            ...
            stmt = select(Planet.id).where(Planet.owner_id == user.id, Planet.type == "personal").limit(1)
            res = await db.execute(stmt)
            ...
            if personal_planet_id:
                ...
                await db.execute(
                    update(AIQuery)
                    .where(AIQuery.user_id == user.id, AIQuery.planet_id == None)
                ...
                await db.execute(
                    update(AIHistory)
                    .where(AIHistory.user_id == user.id, AIHistory.planet_id == None)
                    .values(planet_id=personal_planet_id)
                )
