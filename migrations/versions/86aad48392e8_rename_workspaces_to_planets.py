...
def upgrade() -> None:
    ...
    # Create new indexes
    op.create_index('idx_planets_owner_id', 'planets', ['owner_id'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_planets_type', 'planets', ['type'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_planets_is_active', 'planets', ['is_active'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_planets_last_accessed', 'planets', ['last_accessed'], unique=False)
    op.create_index('idx_planet_members_planet_user', 'planet_members', ['planet_id', 'user_id'], unique=True)
    op.create_index('idx_dashboards_planet_id', 'dashboards', ['planet_id'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    
    # Drop old foreign key constraints and recreate with new names
    op.drop_constraint('workspace_members_workspace_id_fkey', 'planet_members', type_='foreignkey')
    op.drop_constraint('workspace_members_user_id_fkey', 'planet_members', type_='foreignkey')
    op.drop_constraint('dashboards_workspace_id_fkey', 'dashboards', type_='foreignkey')
    
    # Create new foreign key constraints
    op.create_foreign_key('planet_members_planet_id_fkey', 'planet_members', 'planets', ['planet_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key('planet_members_user_id_fkey', 'planet_members', 'users', ['user_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key('dashboards_planet_id_fkey', 'dashboards', 'planets', ['planet_id'], ['id'], ondelete='CASCADE')
    
    # Rename unique constraint
    op.drop_constraint('uq_workspace_members_workspace_user', 'planet_members', type_='unique')
...
def downgrade() -> None:
    ...
    op.create_unique_constraint('uq_workspace_members_workspace_user', 'planet_members', ['planet_id', 'user_id'])
    ...
    op.create_foreign_key('dashboards_workspace_id_fkey', 'dashboards', 'workspaces', ['workspace_id'], ['id'], ondelete='CASCADE')
