...
def upgrade() -> None:
    ...
    if not _column_exists("users", "sso_metadata"):
        op.add_column(
            "users", sa.Column("sso_metadata", postgresql.JSON, nullable=True)
        )
    ...
    if not _index_exists("idx_users_auth0_id", "users"):
        op.create_index(
            ...
            ["auth0_id"],
            unique=True,
            postgresql_where=sa.text("auth0_id IS NOT NULL AND deleted_at IS NULL"),
        )
    ...
    if not _index_exists("idx_users_auth_provider_id", "users"):
        op.create_index(
            ...
            postgresql_where=sa.text(
                "auth_provider_id IS NOT NULL AND deleted_at IS NULL"
            ),
        )
