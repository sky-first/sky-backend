...
def upgrade() -> None:
    ...
    op.add_column('connection_permissions', sa.Column('user_id', sa.UUID(), nullable=True))
    
    # 2. Add foreign key for user_id
    op.create_foreign_key(
        'connection_permissions_user_id_fkey',
        'connection_permissions', 'users',
        ['user_id'], ['id'],
        ondelete='CASCADE'
    )
    ...
    op.drop_constraint('uq_connection_permissions', 'connection_permissions', type_='unique')
...
def downgrade() -> None:
    # 1. Drop index
    op.drop_index('idx_connection_permissions_user_id', table_name='connection_permissions')
    ...
    # 3. Drop foreign key
    op.drop_constraint('connection_permissions_user_id_fkey', 'connection_permissions', type_='foreignkey')
