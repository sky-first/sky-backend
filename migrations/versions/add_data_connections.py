...
def upgrade() -> None:
    ...
    op.create_index('idx_data_connections_connector_id', 'data_connections', ['connector_id'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_data_connections_status', 'data_connections', ['status'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_data_connections_created_by', 'data_connections', ['created_by'], unique=False, postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_data_connections_last_sync', 'data_connections', ['last_sync'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('last_metadata_update', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['connection_id'], ['data_connections.id'], ondelete='CASCADE'),
        sa.UniqueConstraint('connection_id', name='uq_connection_metadata_connection_id')
    )
    op.create_index('idx_connection_metadata_connection_id', 'connection_metadata', ['connection_id'], unique=False)
