...
async def check_data():
    async with AsyncSessionLocal() as db:
        ...
        for s in spaces:
            ...
            res_conn = await db.execute(select(SpaceConnection).where(SpaceConnection.space_id == s.id))
            ...
            for c in conns:
                ...
                # Check metadata
                res_meta = await db.execute(select(ConnectionMetadata).where(ConnectionMetadata.connection_id == c.connection_id))
