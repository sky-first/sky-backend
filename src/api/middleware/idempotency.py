...
async def idempotency_middleware(request: Request, call_next: Callable) -> Response:
    ...
    try:
        ...
        user_id = (
            getattr(request.state, "user_id", None) if hasattr(request.state, "user_id") else "anonymous"
        )
        ...
        if cached_data:
            ...
            try:
                ...
                logger.warning(f"Failed to parse cached idempotency data for key {idempotency_key}: {e}")
