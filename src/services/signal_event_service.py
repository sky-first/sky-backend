...
class SignalEventService:
    ...
    async def list_events(self, space_id: Optional[UUID] = None, crew_id: Optional[UUID] = None) -> List[SignalEventResponse]:
        filters = {}
        if space_id:
    ...
    async def get_event(self, event_id: UUID, space_id: Optional[UUID] = None, crew_id: Optional[UUID] = None) -> SignalEventResponse:
        event = await self.repository.get_by_id(event_id)
        if not event or (space_id and event.space_id != space_id) or (crew_id and event.crew_id != crew_id):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Signal Event not found")
        return event

    async def create_event(self, schema: SignalEventCreate, space_id: Optional[UUID] = None, crew_id: Optional[UUID] = None) -> SignalEventResponse:
    ...
    async def update_event(self, event_id: UUID, schema: SignalEventUpdate, space_id: Optional[UUID] = None, crew_id: Optional[UUID] = None) -> SignalEventResponse:
    ...
    async def delete_event(self, event_id: UUID, space_id: Optional[UUID] = None, crew_id: Optional[UUID] = None) -> None:
        # First check ownership
        await self.get_event(event_id, space_id=space_id, crew_id=crew_id)
